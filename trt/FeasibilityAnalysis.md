...
1. 网络结构可视化（__.pt->.onnx__）
   __Netron__:核对网络计算图，各算子的连接、属性、输入输出等 [Netron](https://netron.app/)  
    ...
    |:--:|:--:|:--:|:--|
    |model1|![var](../s/model1_pt.png)|![var](../s/model1_onnx.png)|一对一:   _convolution->Conv,instance_norm->InstanceNormalization...|
    |model2|![var](../s/model2_pt.png)|![var](../s/model2_onnx.png)|一对多: group_norm ->InstanceNormalization,Mul,Add...|
2. 推理输出误差计算（__.pt->.onnx__）
   ```
    ...
    model = torch.jit.load("model.pt") # pytorch模型加载

    ort_session = ort.InferenceSession('heart.onnx') # onnx模型加载
...
## 结论
1. 模型转换后，在面对请求并发量为1的推理请求的处理上，并不存在明显性能优势
1. 当面临高并发的请求时，TensorRT开始展现出优越性，吞吐量有了明显提升，同等时间内执行的推理请求在使用了动态批处理机制后，吞吐量翻倍。
1. 适用于处理客户端高并发请求。